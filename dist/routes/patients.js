...
const patientSchema = zod_1.z.object({
    ...
    species: zod_1.z.enum(['canino', 'felino', 'ave', 'roedor', 'reptil', 'otro']),
    breed: zod_1.z.string().optional(),
    birthDate: zod_1.z.string().optional(),
    sex: zod_1.z.enum(['macho', 'hembra']).optional(),
    sterilized: zod_1.z.boolean().optional(),
    weightKg: zod_1.z.number().positive().optional(),
    status: zod_1.z.enum(['activo', 'seguimiento', 'control', 'archivado']).optional(),
...
const querySchema = zod_1.z.object({
    ...
    status: zod_1.z.enum(['activo', 'seguimiento', 'control', 'archivado']).optional(),
    species: zod_1.z.enum(['canino', 'felino', 'ave', 'roedor', 'reptil', 'otro']).optional(),
...
async function patientRoutes(app) {
    ...
    app.get('/', auth, async (req, reply) => {
        ...
        const [patients, total] = await Promise.all([
            prisma_js_1.prisma.patient.findMany({
                where,
                skip,
                take: limit,
            ...
            prisma_js_1.prisma.patient.count({ where }),
        ]);
    ...
    app.post('/', auth, async (req, reply) => {
        ...
        const patient = await prisma_js_1.prisma.patient.create({
            data: {
                ...body.data,
    ...
    app.patch('/:id', auth, async (req, reply) => {
        ...
        if (!body.success)
            return reply.status(400).send({ error: body.error.flatten() });
        const existing = await prisma_js_1.prisma.patient.findFirst({
            where: { id, tenantId: req.user.tenantId },
        });
        if (!existing)
            return reply.status(404).send({ error: 'Paciente no encontrado.' });
        const patient = await prisma_js_1.prisma.patient.update({
            where: { id },
            data: {
                ...body.data,
                birthDate: body.data.birthDate ? new Date(body.data.birthDate) : undefined,
    ...
    app.delete('/:id', auth, async (req, reply) => {
        ...
        if (!existing)
            return reply.status(404).send({ error: 'Paciente no encontrado.' });
        await prisma_js_1.prisma.patient.update({ where: { id }, data: { status: 'archivado' } });
