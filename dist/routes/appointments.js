...
const appointmentSchema = zod_1.z.object({
    ...
    type: zod_1.z.enum(['consulta_general', 'vacunacion', 'cirugia', 'control', 'urgencia', 'desparasitacion', 'otro']),
    notes: zod_1.z.string().optional(),
});
const querySchema = zod_1.z.object({
    date: zod_1.z.string().optional(), // YYYY-MM-DD — filtra citas de ese día
    from: zod_1.z.string().optional(),
    to: zod_1.z.string().optional(),
    status: zod_1.z.enum(['pendiente', 'confirmada', 'en_espera', 'completada', 'urgente', 'cancelada']).optional(),
...
async function appointmentRoutes(app) {
    ...
    app.get('/', auth, async (req, reply) => {
        ...
        const [appointments, total] = await Promise.all([
            ...
            prisma_js_1.prisma.appointment.count({ where }),
        ]);
    ...
    app.get('/:id', auth, async (req, reply) => {
        const { id } = req.params;
        const appt = await prisma_js_1.prisma.appointment.findFirst({
    ...
    app.patch('/:id', auth, async (req, reply) => {
        ...
        if (!body.success)
            return reply.status(400).send({ error: body.error.flatten() });
        const existing = await prisma_js_1.prisma.appointment.findFirst({
            where: { id, tenantId: req.user.tenantId },
        });
        if (!existing)
            return reply.status(404).send({ error: 'Cita no encontrada.' });
        const appt = await prisma_js_1.prisma.appointment.update({
            where: { id },
            data: {
                ...body.data,
    ...
    app.delete('/:id', auth, async (req, reply) => {
        ...
        if (!existing)
            return reply.status(404).send({ error: 'Cita no encontrada.' });
        await prisma_js_1.prisma.appointment.update({ where: { id }, data: { status: 'cancelada' } });
